...
class CreateMealTable:
    def __init__(self, root):
            self.root = root
            self.w_dir = os.getcwd()
            self.root.title("Meal Management System | Create Meal Table")
            ...
            # ============= Form ======================
            self.var_monthname = StringVar()
            self.var_password = StringVar()

            title = Label(self.root, text="Create Meal Table", font=("Bahnschrift", 18, "bold"), padx=100, pady=25, bd=5, relief=RIDGE, foreground="#00B0F0")
            ...
            #....Month Name....#
            self.monthname = Entry(frame, textvariable=self.var_monthname, width=25, fg="black", border=0, bg="white", font=("Microsoft YaHei UI Light", 11))
            ...
            #....Password....#
            self.password = Entry(frame, textvariable=self.var_password, width=25, fg="black", border=0, bg="white", font=("Microsoft YaHei UI Light", 11))
            ...
            #.....Submit....#
            Button(frame, command=self.login ,text="Create", padx=40, pady=8, border=0, background="#880015", foreground="white", font=("Bahnschrift", 15)).place(x = 120, y = 160)
    ...
    def login(self):
        if self.monthname.get() == "" or self.password.get() == "":
            ...
            cursor = conn.cursor()
            cursor.execute(f"insert into manager_info(month_name, password) values('"+self.var_monthname.get()+"', '"+self.var_password.get()+"')")
            conn.commit()
